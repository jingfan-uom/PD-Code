...
T = bc_funcs.apply_bc_dirichlet_mirror(T, ghost_inds_right, interior_inds_right,
                                           T_bc=500.0, axis=1,)
T = bc_funcs.apply_bc_zero_flux(T, ghost_inds_left, interior_inds_left,
                                            axis=1,)
T = bc_funcs.apply_bc_zero_flux(T, ghost_inds_top, interior_inds_top, axis=0)
...
save_times = [2,4,6,8,10]  # 单位：小时
save_steps = [int(t * 3600 / dt) for t in save_times]  # 保存时刻对应的步数
T_record = []  # 存储保存时刻的温度场
...
for step in range(nsteps):
    ...
    if step in save_steps:
        T_record.append(T.copy())  # 保存该时刻的温度场
...
"""for i, T_snap in enumerate(T_record):
    sim_time = save_times[i] * 3600  # 转换为秒
