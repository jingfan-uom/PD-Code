...
import Physical_Field_Calculation as pfc  # 导入包含三个函数的模块
# ------------------------
...
def compute_accelerated_velocity( Ur_curr, Uz_curr):
    """
    调用 Physical_Field_Calculation 中的三个函数计算整体位移场。
    """
    Ur_new = Ur_curr
    Uz_new = Uz_curr
    ...
    Ar_new = np.sum(Ar_new , axis=1).reshape(Ur_curr.shape)  # shape 和 Ur_curr 一致
    Az_new = np.sum(Az_new, axis=1).reshape(Uz_curr.shape)

    return  Ar_new,Az_new   # 或者返回你想要的数据
...
# ------------------------
Ur = np.zeros_like(Rmat)  # 径向位移
Uz = np.zeros_like(Rmat)  # 轴向位移

Vr = np.zeros_like(Rmat)  # 径向速度
Vz = np.zeros_like(Rmat)  # 轴向速度

Ar = np.zeros_like(Rmat)  # 径向加速度
Az = np.zeros_like(Rmat)  # 轴向加速度
...
total_time = 100  # Total simulation time (5 hours)
