...
def compute_next_displacement_field(Ur_curr, Uz_curr, Vr_curr, Vz_curr, Ar_new, Az_new, dt_m):
...
def compute_next_velocity_third_step(Vr_half, Vz_half, Ur_next, Uz_next, dt):
    Ar_next, Az_next = compute_accelerated_velocity(Ur_next, Uz_next, Rmat, Zmat,horizon_mask,dir_r ,dir_z ,c ,partial_area_matrix, rho_s, bz)
...
for i in range(5):
    ...
    Ur, Uz, Vr_half, Vz_half = compute_next_displacement_field(Ur, Uz, Vr, Vz, Ar, Az, dt_m)
    Vr, Vz, Ar, Az = compute_next_velocity_third_step(Vr_half, Vz_half, Ur, Uz, dt_m)
