...
def compute_direction_matrix(X, Y, horizon_mask):
    """
    Compute the direction matrix based on the small deformation assumption.

    Inputs:
    - X, Y: coordinate grids (Ny, Nx)
    - horizon_mask: mask matrix (N, N), indicates whether particle pairs are within interaction range

    Outputs:
    - dir_x: unit vector in r-direction (cosine), shape (N, N)
    - dir_z: unit vector in z-direction (sine), shape (N, N)
    """
    ...
    distance[~horizon_mask] = 1.0  # Avoid division by zero
