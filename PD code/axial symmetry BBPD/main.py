...
# ==========================
#  Plot: ghosts (no corners) + physical points
# ==========================
import numpy as np
import matplotlib.pyplot as plt

# 为避免 1D 情形下 Rmat/Zmat 维度不匹配，这里单独建绘图网格
if (np.ndim(Rmat) == 1) or (np.ndim(Zmat) == 1):
    Rplot, Zplot = np.meshgrid(r_all, z_all, indexing='xy')
else:
    Rplot, Zplot = Rmat, Zmat

# 基于坐标范围的原始边界判定
raw_top    = (Zplot >  Lz + tolerance)
raw_bottom = (Zplot <  0.0 - tolerance)
raw_left   = (Rplot <  0.0 - tolerance)
raw_right  = (Rplot >  Lr + tolerance)

# 角落（同时满足上下与左右之一）
corner_mask = (raw_top & raw_left) | (raw_top & raw_right) | \
              (raw_bottom & raw_left) | (raw_bottom & raw_right)

# 去掉角落后的边界掩码；左右边再去掉与上下的重叠
mask_top    =  raw_top & ~corner_mask
mask_bottom =  raw_bottom & ~corner_mask
mask_left   =  raw_left & ~corner_mask & ~(raw_top | raw_bottom)
mask_right  =  raw_right & ~corner_mask & ~(raw_top | raw_bottom)

# 物理域点（既不在上/下/左/右 ghost 带）
mask_physical = ~(raw_top | raw_bottom | raw_left | raw_right)

# 绘图
fig, ax = plt.subplots(figsize=(6, 6))
# 物理点先画，放在底层
ax.scatter(Rplot[mask_physical], Zplot[mask_physical], s=8, c='0.75',
           label='Physical points', zorder=1)

# 各边界（不同颜色）
ax.scatter(Rplot[mask_top],    Zplot[mask_top],    s=14, c='tab:red',    label='Top ghost',    zorder=2)
ax.scatter(Rplot[mask_bottom], Zplot[mask_bottom], s=14, c='tab:blue',   label='Bottom ghost', zorder=2)
ax.scatter(Rplot[mask_left],   Zplot[mask_left],   s=14, c='tab:green',  label='Left ghost',   zorder=2)
ax.scatter(Rplot[mask_right],  Zplot[mask_right],  s=14, c='tab:orange', label='Right ghost',  zorder=2)

# 物理域边框（可选）
ax.plot([0, Lr, Lr, 0, 0], [0, 0, Lz, Lz, 0], 'k--', lw=1, alpha=0.6)

ax.set_xlabel('r'); ax.set_ylabel('z')
ax.set_aspect('equal', adjustable='box')
ax.grid(True, linestyle='--', alpha=0.4)
# 放置图例到 (3/2*Lr, Lz/2) 的数据坐标
# 图例放在 (1.5*Lr, 0.5*Lz) 的数据坐标
x_leg = 1.35 * Lr
y_leg = 0.5 * Lz
ax.legend(loc='center', bbox_to_anchor=(x_leg, y_leg),
          bbox_transform=ax.transData, frameon=True)

# 将 r 轴上限扩到 2*Lr（保持左端不变，以包含左侧 ghost）
xmin, xmax = ax.get_xlim()
ax.set_xlim(xmin, max(1.7 * Lr, xmax))
...
Vr_half = ADR.adr_initial_velocity(Fr_0, 1 , lambda_diag_matrix)
Vz_half = ADR.adr_initial_velocity(Fz_0, 1 , lambda_diag_matrix)
Ur = Vr_half * 1 + Ur
Uz = Vz_half * 1 + Uz
