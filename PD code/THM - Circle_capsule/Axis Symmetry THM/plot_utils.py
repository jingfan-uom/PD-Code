...
def plot_temperature_contour_in_circle(phys_coords_list,dr, T_phys, radius, cmap, title='Temperature Contour',
                                       levels=20):
    """
    ...
    level_values = np.linspace(vmin, vmax, 32)
    ...
    cbar.set_label(
        f"Temperature (K)\nMin: {vmin:.2f} K\nMax: {vmax:.2f} K",
        rotation=270, labelpad=10, va='bottom'
    )
