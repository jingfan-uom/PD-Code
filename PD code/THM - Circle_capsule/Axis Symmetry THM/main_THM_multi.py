...
if __name__ == "__main__":
    ...
    for (region_id, direction), neighbor_data in boundary_neighbors_t.items():
        if direction == 'right':
            ...
            T_right[region_id][ghost_indices] = T_increment  # 你现在用的是 Dirichlet
    ...
    def find_closest_point_in_phys(phys_coords_list, target_r, target_z, n_slices):
        best_slice = None
        best_local_idx = None
        best_coord = None
        best_dist2 = np.inf
    ...
    for name, target_r, target_z in target_points:
        slc, idx, coord = find_closest_point_in_phys(
            phys_coords_list_t, target_r, target_z, n_slices
        )
        monitor_points[name] = {
            "slice": slc,
            "idx": idx,
        ...
        print(f"{name} -> slice={slc}, idx={idx}, coord={coord}")
    ...
    for step1 in range(nsteps_th):
        ...
        # 监测两个点的温度差
        slice_a = monitor_points["(2dr, r)"]["slice"]
        idx_a = monitor_points["(2dr, r)"]["idx"]

        slice_b = monitor_points["(r/2, r)"]["slice"]
        idx_b = monitor_points["(r/2, r)"]["idx"]

        Ta = T_phys[slice_a][idx_a]
        Tb = T_phys[slice_b][idx_b]

        if abs(Ta - Tb) < 0.1 and Ta > Tsurr - 10:
            print(f"Temperature loop stopped at step {step1}, |Ta-Tb|={abs(Ta - Tb):.6f}")
            break
        ...
        for (region_id, direction), neighbor_data in boundary_neighbors_t.items():
            if direction == 'right':
                ...
                ghost_indices = neighbor_data["ghost_indices"]
                phys_indices = neighbor_data["phys_indices"]  # 这些索引是相对于 phys_coords 的
                T_right[region_id][ghost_indices] = 2 * T_increment - T1[phys_indices]  # 你现在用的是 Dirichlet
                T[region_id] = np.concatenate(
                    [T_phys[region_id], T_left[region_id], T_right[region_id], T_top[region_id], T_bot[region_id]])
        ...
        if deltaV_total_all > cavity_vol:
            cracking_act = True
            print("Cavity is full!")

        if f_void > 1e-5:
            print(f" Volume proportion :{deltaV_total_all / cavity_vol * 100} %")
        """ 空腔属性的update"""
        ...
        if deltaV_total_all <= cavity_vol:
            ...
            p = abs(E_shell / (3 * (1 - 2 * nu_shell)) * (deltaV_phase - cavity_vol) / V_shell)
        ...
        if step1 == 0:
            for i in range(n_slices):
                ...
                dr_i = zones[i]["dr"]
                br[i][idx_local] = abs(p * unit_vec[:, 0])
                bz[i][idx_local] = abs(p * unit_vec[:, 1])
    ...
    phi_list = [phi_regions[i] for i in range(n_slices)]
    plot.plot_mu_field(r, coords_all_m_list, phi_list, grid_n=300, title_prefix="Degree of damage")
