...
def compute_thermal_conductivity_matrix(Tarr, k_s, k_l, T_solidus, T_liquidus, delta, true_indices):

    lambda_flat = get_thermal_conductivity(Tarr, k_s, k_l, T_solidus, T_liquidus, delta)
    N = len(Tarr)
...
def build_K_matrix(Tarr, compute_thermal_conductivity_matrix, factor_mat,
                   ...
                   distance_matrix, horizon_mask, true_indices, k_s, k_l, T_solidus, T_liquidus, delta, dt):
    N = len(Tarr)
    cond_mat = compute_thermal_conductivity_matrix(Tarr, k_s, k_l, T_solidus, T_liquidus, delta, true_indices)
    K1 = np.zeros((N, N))
