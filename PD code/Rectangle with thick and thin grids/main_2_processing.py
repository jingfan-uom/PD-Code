...
def update_temperature(Tcurr, Hcurr, Kmat, Nz_all, Nr_all,
                       ...
                       delta, r_flat, dt, T_copy, coord_index,coarse
                       ):
...
def update_coarse(args):
    return update_temperature(*args, coarse=1)

def update_fine(args):
    return update_temperature(*args, coarse=0)
if __name__ == "__main__":
    ...
    r_ghost_left_coarse = np.linspace(r_start_coarse - ghost_nodes_r * dr_coarse + dr_coarse / 2,
                                      r_start_coarse - dr_coarse / 2, ghost_nodes_r)
    ...
    z_ghost_top_coarse = np.linspace(z_start_coarse + Lz_coarse + (ghost_nodes_z - 1) * dz_coarse + dz_coarse / 2,
                                     ...
                                     ghost_nodes_z)
    z_ghost_bot_coarse = np.linspace(z_start_coarse - dz_coarse / 2,
                                     z_start_coarse - ghost_nodes_z * dz_coarse + dz_coarse / 2, ghost_nodes_z)
    ...
    ghost_inds_bottom_coarse, interior_inds_bottom_coarse = bc_funcs.get_bottom_ghost_indices(z_all_coarse,
                                                                                              ghost_nodes_z)
    ...
    r_ghost_left_fine = np.linspace(r_start_fine - ghost_nodes_r * dr_fine + dr_fine / 2, r_start_fine - dr_fine / 2,
                                    ghost_nodes_r)
    ...
    z_ghost_top_fine = np.linspace(z_start_fine + Lz_fine + (ghost_nodes_z - 1) * dz_fine + dz_fine / 2,
                                   ...
                                   ghost_nodes_z)
    z_ghost_bot_fine = np.linspace(z_start_fine - dz_fine / 2, z_start_fine - ghost_nodes_z * dz_fine + dz_fine / 2,
                                   ghost_nodes_z)
    ...
    factor_mat_fine = np.where(distance_matrix_fine <= threshold_distance_fine + tolerance, 1.125,
                               1.0)  # Local adjustment factor
    factor_mat_coarse = np.where(distance_matrix_coarse <= threshold_distance_coarse + tolerance, 1.125,
                                 1.0)  # Local adjustment factor
    ...
    Kmat_coarse = cf.build_K_matrix(T_coarse, cf.compute_thermal_conductivity_matrix, factor_mat_coarse,
                                    ...
                                    distance_matrix_coarse, horizon_mask_coarse, true_indices_coarse, ks, kl, Ts, Tl,
                                    delta_coarse, r_flat_coarse,
                                    dt)
    Kmat_fine = cf.build_K_matrix(T_fine, cf.compute_thermal_conductivity_matrix, factor_mat_fine,
                                  ...
                                  distance_matrix_fine, horizon_mask_fine, true_indices_fine, ks, kl, Ts, Tl,
                                  delta_fine, r_flat_fine,
    ...
    for step in range(nsteps):
        args_coarse = (
            ...
            delta_coarse, r_flat_coarse, dt,
            T_fine.copy(), coord_index_coarse,
            1
        )
        args_fine = (
            T_fine, H_fine, Kmat_fine, Nz_all_fine, Nr_all_fine,
            ...
            delta_fine, r_flat_fine, dt,
            T_coarse.copy(), coord_index_left_fine,
            0
        )
        with Pool(processes=2) as pool:
            result = pool.starmap_async(update_temperature, [args_coarse, args_fine])
            (T_coarse, H_coarse, Kmat_coarse), (T_fine, H_fine, Kmat_fine) = result.get()
