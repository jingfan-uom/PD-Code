...
Nr, Nz = 60, 40         # Number of cells in r and z directions
...
# ------------------------r
r_start = 0.1  # Starting position in r-direction
tolerance = 1e-8
...
dilation_r = np.zeros_like(distance_matrix, dtype=float)
...
def compute_accelerated_velocity(Ur, r_flat, lamda, a,  horizon_mask, distance_matrix, partial_area_matrix,
                                  dilation, eij, n_x, n_y, bz):
    """
    ...
    bulk = (theta_i + theta_j) * lamda + (Ur_i + Ur_j)/r_flat * (lamda + a) # N×1 广播到 N×N

    dforce[horizon_mask] = partial_area_matrix[horizon_mask] * (
                3 / delta ** 3 / np.pi * bulk[horizon_mask] +
                miu * 24 / delta ** 3 / np.pi * eij[horizon_mask] / distance_matrix[horizon_mask]) \
                           * scr[horizon_mask] * shape_factor_matrix[horizon_mask]

    fx[horizon_mask] = n_x[horizon_mask] * dforce[horizon_mask]
    fy[horizon_mask] = n_y[horizon_mask] * dforce[horizon_mask]
    br = -2 / r_flat * (dilation * (lamda + a) + (lamda + gama) * Ur_i.reshape(-1) / r_flat)
    Fx_new = np.sum(fx, axis=1) + br
    Fy_new = np.sum(fy, axis=1) + bz   # 加上体力
...
massvec = 0.25 * (np.pi * delta**2 * h) * (4 * delta * 6.0 * miu / (np.pi * h * delta**4) / dr) * 1.1
mass = np.ones_like(Ur) * massvec  # 形状自动匹配 U

# ------------------------
# Simulation loop settings
# ------------------------
eij = np.zeros_like(distance_matrix)
term = np.zeros_like(distance_matrix)
...
for step in range(nsteps):
    ...
    eij[horizon_mask] = nlength[horizon_mask] - distance_matrix[horizon_mask]
    ...
    term[horizon_mask] = eij[horizon_mask] * partial_area_matrix[horizon_mask] * 3 / delta**3 / np.pi * shape_factor_matrix[horizon_mask] \

    # 对 j 求和得到每个 i 的 θ_i
    dilation = np.sum(term, axis=1)

    if step == 0:
        Fr_0, Fz_0 = compute_accelerated_velocity(Ur, r_flat, lamda, a, horizon_mask, distance_matrix, partial_area_matrix,
                                                   dilation, eij, n_r, n_z, bz)
        ...
        previous_Ur = Ur
        previous_Uz = Uz
        Fr, Fz = compute_accelerated_velocity(Ur, r_flat, lamda, a, horizon_mask, distance_matrix, partial_area_matrix,
                                                   dilation, eij, n_r, n_z, bz)
