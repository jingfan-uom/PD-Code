...
def get_enthalpy(
    ...
    if np.any(mask_core):
        ...
        if np.any(mask_phase):
            ...
            dcp = Cl - Cs
            a0 = rho_s * (Cs + L / dT)
            a1 = rho_s * dcp + drho * (Cs + L / dT)
            a2 = drho * dcp
            phase_int = dT * (
                a0 * alpha + 0.5 * a1 * alpha * alpha + (a2 / 3.0) * alpha * alpha * alpha
            )
            Hc[mask_phase] = rho_s * Cs * Ts + phase_int
        ...
        if np.any(mask_liquid):
            ...
            a1 = rho_s * dcp + drho * (Cs + L / dT)
            a2 = drho * dcp
            phase_full = dT * (a0 + 0.5 * a1 + a2 / 3.0)
            H_Tl = rho_s * Cs * Ts + phase_full
            Hc[mask_liquid] = H_Tl + rho_l * Cl * (T_core[mask_liquid] - Tl)
        H[mask_core] = Hc
...
def temperature_from_enthalpy_numba(
    ...
    a2 = drho * dcp
    phase_full = dT * (a0 + 0.5 * a1 + a2 / 3.0)
    H_liquid_min = H_solid_max + phase_full
    ...
    for i in prange(N):
        ...
        if mask_core[i]:
            if H <= H_solid_max:
                ...
                target = H - H_solid_max
                lo = 0.0
                hi = 1.0
                for _ in range(40):
                    mid = 0.5 * (lo + hi)
                    phase_mid = dT * (
                        a0 * mid + 0.5 * a1 * mid * mid + (a2 / 3.0) * mid * mid * mid
                    )
                    if phase_mid < target:
                        lo = mid
                    else:
