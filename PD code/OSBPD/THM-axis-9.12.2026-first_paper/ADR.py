...
@njit(parallel=True, fastmath=True)
def compute_lambda_diag_matrix_axsy_node(
    indptr, indices,
    area_edge,
    dist_edge,
    w_maxe_edge,
    ...
    for i in prange(N):
        ...
        for p in range(a0, b0):
            ...
            Kt_i = (
                4.0 * omega * lam_i / q
                + ((lam_i + alpha_i) * (2.0 * delta / q) * inv_ri)
                + (beta_i * omega) * shp * 2.0 * np.pi
            )

            Kt_j = (
                4.0 * omega * lam_j / q
                + ((lam_j + alpha_j) * (2.0 * delta / q) * inv_rj)
                + (beta_j * omega) / shp * 2.0 * np.pi
            )
        ...
        Kb = (
            (lam_i + gamma_i) * (inv_ri * inv_ri)
            + 4.0 * delta * np.pi * Kb_i * inv_ri * (lam_i + alpha_i) / q
        )
        diagKt[i] = 0.25 * (acc + np.abs(Kb))
