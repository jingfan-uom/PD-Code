...
def find_mirror_pairs(coords_ghost, coords_phys, tol, r_axis=0.0):
    ghost_indices = []
    phys_indices = []

    for i, (xg, zg) in enumerate(coords_ghost):
        mirror_coord = (2.0 * r_axis - xg, zg)
        ...
        if dists[idx_phys] < tol:
            ...
            raise ValueError(
                f"No matching physical point found for ghost point {i} at {mirror_coord}, "
                f"min dist = {dists[idx_phys]:.3e}"
            )

    return np.array(ghost_indices, dtype=int), np.array(phys_indices, dtype=int)

def find_circle_mirror_pairs_multilayer(
        coords_ghost_circle,
        coords_phys,
        dr,
        R,
    ...
    Circle center: (r_start, R)
    """
    ghost_indices = []
    phys_indices = []
    coords_ghost_circle = np.asarray(coords_ghost_circle, dtype=np.float64)
    coords_phys = np.asarray(coords_phys, dtype=np.float64)
    if coords_ghost_circle.size == 0:
        return np.empty(0, dtype=int), np.empty(0, dtype=int)
    if coords_phys.size == 0:
        raise ValueError("coords_phys is empty; cannot build mirror pairs for right ghost nodes")
    ...
    for i, (xg, zg) in enumerate(coords_ghost_circle):
        if modify_coordinates:
            dx = xg - r_center
            dz = zg - z_center
            level = max((dx / axis_x) ** 2 + (dz / axis_z) ** 2, 1.0e-30)
            scale = 1.0 / np.sqrt(level)
            xb = r_center + dx * scale
            zb = z_center + dz * scale
            n = geom.capsule_outward_normals(
                np.array([xb]), np.array([zb]),
                r_center, z_center,
                axis_x, axis_z,
            )[0]
            signed_distance = max(
            ...
            mirror_point = np.array([xg, zg]) - 2.0 * signed_distance * n
        ...
        # distance from shifted circle center
        r_ghost = np.sqrt((xg - r_center) ** 2 + (zg - z_center) ** 2)
        k = int(round((r_ghost - R) / dr))

        # outward normal from shifted center
        n = np.array([xg - r_center, zg - z_center]) / r_ghost

        # reflect inward across circular boundary
