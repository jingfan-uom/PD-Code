...
def plot_temperature_contour_in_circle(
    ...
    T_rmin = griddata(all_coords, all_temps, (r_start + dr / 2, radius), method='linear')
    T_zmin = griddata(all_coords, all_temps, (r_start + dr / 2, dr / 2), method='linear')
    T_zmax = griddata(all_coords, all_temps, (r_start + dr / 2, 2.0 * radius - dr / 2), method='linear')
    T_zmax1 = griddata(all_coords, all_temps, (r_start + radius - dr / 2, radius), method='linear')
    ...
    # 5. Mask processing: shifted semicircle
    mask_circle = (
        ((r_grid - r_start) ** 2 + (z_grid - radius) ** 2 <= radius ** 2) &
        (r_grid >= r_start) &
        (z_grid >= 0) &
        (z_grid <= 2 * radius)
    )
    T_grid[~mask_circle] = np.nan
    ...
    ax.set_xlim([r_start, r_start + radius])
...
def plot_displacement_contours_in_circle(
    ...
    r_lin = np.linspace(r_start, r_start + radius, grid_n)
    ...
    # 5) Mask: keep only shifted semicircle
    mask_circle = (
        ((r_grid - r_start) ** 2 + (z_grid - radius) ** 2 <= radius ** 2) &
        (r_grid >= r_start + dr1 / 2) &
        (z_grid >= dr1 / 2) &
...
def plot_mu_field(R, coords_list, phi_list, grid_n=300, title_prefix=""):
    ...
    cbar.set_label(f"{title_prefix} ", fontsize=12)
