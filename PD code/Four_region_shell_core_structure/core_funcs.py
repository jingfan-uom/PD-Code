...
def get_enthalpy(Tarr, mask_core, rho_s, rho_l, Cs, Cl, L, T_solidus, T_liquidus, rho_shell, Cshell):
    if mask_core:
...
def get_temperature(Harr, mask_core, rho_s, rho_l, Cs, Cl, L, T_solidus, T_liquidus, rho_shell, Cshell):
    if mask_core:
...
def find_shell_interface(mask_core):
    """
    Identify shell-side interface points at the core-shell boundary, including the outermost layer.
    Input:
        mask_core: 2D bool array, True = core, False = shell
    Output:
        shell_edge: 2D bool array, True = shell-side interface
    """
    rows, cols = mask_core.shape
    # Pad the boundary with False
    padded = np.pad(mask_core, pad_width=1, mode='constant', constant_values=False)
    shell_edge = np.zeros_like(mask_core, dtype=bool)
    for i in range(rows):
        for j in range(cols):
            if not mask_core[i, j]:  # Shell region
                # 3x3 neighborhood; shift i, j by +1 to align with padded indices
                local = padded[i:i+3, j:j+3]
                if np.any(local):  # Neighboring core exists
                    shell_edge[i, j] = True
    return shell_edge
