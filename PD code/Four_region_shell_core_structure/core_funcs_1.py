...
def get_enthalpy(Tarr, rho, cs, cl, L, T_solidus, T_liquidus):
    ...
    # (2) Phase change region
    dT = (T_liquidus - T_solidus)
    alpha = (Tarr[mask_phase] - T_solidus) / dT
    # Sensible heat + partial latent heat: Cs * T_solidus + alpha * L
    H[mask_phase] = rho * (
...
def get_temperature(Harr, rho, Cs, Cl, L, T_solidus, T_liquidus):
    ...
    # (1) Threshold for solid enthalpy region
    H_solid_max = rho * Cs * T_solidus
    # (2) Threshold for phase change region: H_solid_max + rho * L
...
def build_K_matrix(Tarr, compute_thermal_conductivity_matrix, factor_mat,
                   ...
                   k_s, k_l, T_solidus, T_liquidus, delta, dz, dt):
    N = len(r_flat)
    ...
    # Diagonal terms to ensure each row sums to zero
...
def compute_dt_cr_th_solid_with_dist(rho_solid, c_solid, k_solid,
    ...
    i = N // 2  # Select the center particle (for example)
    ...
    distances = np.where(valid_mask, distance_row, 0.0)  # Invalid pairs get 0
    areas = np.where(valid_mask, partial_area_matrix[i], 0.0)

    # Effective thermal conductivity (nonlocal)
    k = k_solid * 4.0 / (np.pi * delta ** 2)

    # Replace 0 or invalid distances with NaN-safe values
