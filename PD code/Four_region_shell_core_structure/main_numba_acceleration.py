...
@jit(nopython=True)
def compute_accelerated_velocity_jit(
    Ur_curr, Uz_curr, mu,
    r_flat_m, z_flat_m, horizon_mask_m, distance_matrix_m,
    dir_r, dir_z, c_matrix, partial_area_matrix_m, rho_s, s0):
    """
    Compute acceleration and update failure matrix in JIT-compatible form.
    """
    # 这里假设 compute_s_matrix 和 update_mu_by_failure 也是 @jit 编译过的
    Relative_elongation = pfc.compute_s_matrix(r_flat_m, z_flat_m, Ur_curr, Uz_curr, horizon_mask_m, distance_matrix_m)
    ...
    Ar_sum = np.sum(Ar_new, axis=1)
    Az_sum = np.sum(Az_new, axis=1)

    return Ar_sum, Az_sum, mu

@jit(nopython=True)
def compute_next_displacement_field_jit(Ur_curr, Uz_curr, Vr_curr, Vz_curr, Ar_new, Az_new, dt_m):
...
@jit(nopython=True)
def compute_next_velocity_third_step_jit(Vr_half, Vz_half, Ur_next, Uz_next, dt_m,
                                         mu, r_flat_m, z_flat_m, horizon_mask_m, distance_matrix_m,
                                         dir_r, dir_z, c_matrix, partial_area_matrix_m, rho_s, s0):
    Ar_next, Az_next, mu = compute_accelerated_velocity_jit(
        Ur_next, Uz_next, mu,
        r_flat_m, z_flat_m, horizon_mask_m, distance_matrix_m,
        dir_r, dir_z, c_matrix, partial_area_matrix_m, rho_s, s0
    )
...
for step1 in range(nsteps_m):
    previous_Ur = Ur.copy()
    previous_Uz = Uz.copy()
    Ar, Az, mu = compute_accelerated_velocity_jit(
        Ur, Uz, mu,
        r_flat_m, z_flat_m, horizon_mask_m, distance_matrix_m,
        dir_r, dir_z, c_matrix, partial_area_matrix_m, rho_s, s0
    )
    ...
    Vr, Vz, Ar, Az = compute_next_velocity_third_step_jit(
        ...
        mu,
        r_flat_m, z_flat_m,
        horizon_mask_m, distance_matrix_m,
        dir_r, dir_z,
        c_matrix, partial_area_matrix_m,
        rho_s, s0
    )
